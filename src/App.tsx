...
export default function App() {
  ...
  const nav = [
    { label: t.dashboard, show: true, active: true },
    { label: t.services, show: role === "admin" },
    { label: t.manuals, show: true },
    { label: t.reviewQueue, show: role === "reviewer" || role === "admin" },
    { label: t.searchManuals, show: true },
    { label: t.auditLog, show: role === "admin" },
  ].filter((x) => x.show);
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-6 px-4 py-6 md:grid-cols-[260px_1fr]">
        ...
        <aside className="h-fit rounded-lg border bg-card">
          ...
          <nav className="p-2 space-y-1">
            {nav.map((x) => (
              <NavItem key={x.label} label={x.label} active={x.active} />
            ))}
          </nav>
        </aside>
